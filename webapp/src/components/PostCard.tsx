...
import { Star, Eye, Wallet, Building2, MapPin } from 'lucide-react';
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 14px', borderTop: '1px solid var(--border)', background: 'var(--bg)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            {[1, 2, 3, 4, 5].map(i => (
              <Star key={i} size={14} fill={i <= rating ? '#f5b731' : 'transparent'} stroke={i <= rating ? '#f5b731' : '#d1d5db'} strokeWidth={1.8} />
            ))}
          </div>
          {post.salary ? (<div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, fontWeight: 700, color: 'var(--gold-dark)' }}><Wallet size={13} strokeWidth={2} /> {post.salary}</div>) : <span />}
          <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--text-muted)' }}><Eye size={12} strokeWidth={1.8} /> {views}</span>
    ...
    <Link href={`/posts/${post.id}`} style={{ display: 'block', textDecoration: 'none' }}>
      <div className="card" style={{ padding: 16 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
          {post.company && <Tag svg={<Building2 size={12} strokeWidth={1.8} />} text={post.company} />}
          {post.city && <Tag svg={<MapPin size={12} strokeWidth={1.8} />} text={post.city} />}
        </div>
        {post.salary && (<div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'var(--gold-light)', color: 'var(--gold-dark)', borderRadius: 8, padding: '5px 12px', fontSize: 12, fontWeight: 700, marginBottom: 8 }}><Wallet size={12} strokeWidth={2} /> {post.salary}</div>)}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 8, borderTop: '1px solid var(--border)', marginTop: 2 }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--text-muted)' }}><Eye size={12} strokeWidth={1.8} /> {(post as any).views || (post as any).viewCount || 0}</span>
