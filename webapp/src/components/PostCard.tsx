...
export default function PostCard({ post }: { post: Post }) {export default function PostCard({ post }: { post: Post }) {
  ...
  if (post.type === 'RESUME') return <ResumeCard post={post} />;  const c = getPostTypeColor(post.type);

  return <DefaultCard post={post} />;  return (

}    <Link href={`/posts/${post.id}`} style={{display:'block',textDecoration:'none'}}>

      <div className="card" style={{padding:16}}>

/* ── RESUME — special card ───────────────────────────── */        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10}}>

function ResumeCard({ post }: { post: Post }) {          <span className="badge" style={{background:c+'14',color:c,fontSize:10}}>{getPostTypeLabel(post.type)}</span>

  const author = post.author;          <span style={{fontSize:11,color:'var(--text-muted)',fontWeight:500}}>{timeAgo(post.createdAt)}</span>

  const photoUrl = author?.photoUrl || author?.photo;        </div>
  ...
  const rating = author?.rating || 0;        <div style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:8}}>

  const views = (post as any).views || (post as any).viewCount || 0;          {post.company && <Tag svg={<svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><rect x="3" y="7" width="18" height="14" rx="2"/><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2"/></svg>} text={post.company}/>}
  ...
  return (        </div>
    ...
    <Link href={`/posts/${post.id}`} style={{ display: 'block', textDecoration: 'none' }}>        {post.salary && (

      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>          <div style={{display:'inline-flex',alignItems:'center',gap:5,background:'var(--gold-light)',color:'var(--gold-dark)',borderRadius:8,padding:'5px 12px',fontSize:12,fontWeight:700,marginBottom:8}}>

        {/* ── Top section ── */}            <svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>

        <div style={{ display: 'flex', gap: 12, padding: '14px 14px 10px' }}>            {post.salary}
          ...
          <div style={{ flexShrink: 0 }}>        )}
            ...
            {photoUrl ? (        {post.technologies && post.technologies.length > 0 && (
              ...
              <img src={photoUrl} alt="" style={{ width: 56, height: 56, borderRadius: 14, objectFit: 'cover', border: '2px solid rgba(184,160,106,0.25)' }} />          <div style={{display:'flex',flexWrap:'wrap',gap:4,marginBottom:8}}>

            ) : (            {post.technologies.slice(0,4).map((t: any) => (

              <div style={{ width: 56, height: 56, borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #1e3a5f, #2d5a8e)', color: '#fff', fontSize: 20, fontWeight: 800 }}>              <span key={typeof t==='string'?t:t.id} style={{fontSize:10,fontWeight:700,padding:'3px 9px',borderRadius:6,background:'var(--navy-light)',color:'var(--navy)'}}>

                {name[0] || '?'}                {typeof t==='string'?t:t.name}
          ...
          <div style={{ flex: 1, minWidth: 0 }}>        )}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 6 }}>        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',paddingTop:8,borderTop:'1px solid var(--border)',marginTop:2}}>

              <div style={{ minWidth: 0 }}>          <span style={{display:'flex',alignItems:'center',gap:4,fontSize:11,color:'var(--text-muted)'}}>

                <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 }}>            <svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>

                  {post.title || author?.profession || 'Mutaxassis'}            {(post as any).views||(post as any).viewCount||0}

                </div>          </span>

                <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--navy)', lineHeight: 1.2 }}>          {post.author && (

                  {name}            <div style={{display:'flex',alignItems:'center',gap:6}}>

                </div>              {post.author.photoUrl || post.author.photo ? (

              </div>                <img src={post.author.photoUrl || post.author.photo} alt="" style={{width:22,height:22,borderRadius:7,objectFit:'cover'}}/>

              <div style={{ textAlign: 'right', flexShrink: 0 }}>              ) : (

                {post.experience && (                <div style={{width:22,height:22,borderRadius:7,display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(135deg,var(--navy),#2a4f7a)',color:'#fff',fontSize:10,fontWeight:800}}>{post.author.firstName?.[0]||'?'}</div>

                  <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>              )}

                    📊 {post.experience}              <span style={{fontSize:11,fontWeight:600,color:'var(--text-secondary)'}}>{post.author.firstName}</span>
            ...
            </div>function Tag({svg,text}:{svg:React.ReactNode;text:string}) {

            {/* Technologies */}  return <span style={{display:'inline-flex',alignItems:'center',gap:4,fontSize:11,fontWeight:500,color:'var(--text-secondary)',background:'var(--bg)',padding:'4px 10px',borderRadius:6}}>{svg} {text}</span>;
        ...
        <div style={{
          ...
          padding: '8px 14px', borderTop: '1px solid var(--border)', background: 'var(--bg)',
        }}>
          {/* Rating stars */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            {[1, 2, 3, 4, 5].map(i => (
              <svg key={i} width="13" height="13" viewBox="0 0 24 24" fill={i <= rating ? 'var(--gold)' : 'none'} stroke={i <= rating ? 'var(--gold)' : '#ccc'} strokeWidth="2">
