    ...
    <Link href={`/posts/${post.id}`} style={{ display: 'block', textDecoration: 'none', minHeight: 44 }}>
      <article
        ...
        onMouseEnter={!m ? (e) => {
          ...
          e.currentTarget.style.boxShadow = '0 24px 48px hsla(217, 91%, 60%, 0.15), 0 0 0 1px hsla(217, 91%, 60%, 0.3)';
        } : undefined}
        onMouseLeave={!m ? (e) => {
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: m ? 10 : 14, gap: m ? 8 : 12 }}>
          <span
            style={{
              ...
              borderRadius: 9999, boxShadow: '0 4px 14px hsla(217, 91%, 60%, 0.5)',
              whiteSpace: 'nowrap',
            }}
          >
            Kurs
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: m ? 6 : 8, marginBottom: m ? 10 : 12 }}>
          {post.company && (
            <span style={{
              ...
              <Briefcase size={m ? 12 : 14} strokeWidth={2} /> {post.company}
            </span>
          )}
          {post.city && (
            <span style={{
        ...
        {techs.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: m ? 4 : 6, marginBottom: m ? 10 : 14 }}>
            ...
            {remainingCount > 0 && (
              <span style={{
                fontSize: m ? 11 : 12, fontWeight: 600, padding: m ? '3px 10px' : '4px 12px',
                borderRadius: 10, background: 'hsla(217, 91%, 60%, 0.1)', color: 'hsl(217, 91%, 60%)',
              }}>
                +{remainingCount}
              </span>
            )}
