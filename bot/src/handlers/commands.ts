...
import { mainKeyboard, subscribeKeyboard, webAppKeyboard } from '../keyboards';
...
export async function handleStart(ctx: Context) {
  ...
  if (!isSubscribed && config.channelId) {
    await ctx.reply(
      `🇺🇿 <b>IT Jobs UZ</b> ga xush kelibsiz!\n\n` +
...
export async function handleHelp(ctx: Context) {
  await ctx.reply(
    `ℹ️ <b>Yordam</b>\n\n` +
    `<b>IT Jobs UZ</b> — O'zbekiston IT sohasidagi eng katta platforma.\n\n` +
    `🔹 <b>Vakansiya qo'yish</b> — kompaniyalar uchun\n` +
    `🔹 <b>Rezyume qo'yish</b> — ish izlovchilar uchun\n` +
    `🔹 <b>Kurs e'lon qilish</b> — o'qituvchilar uchun\n` +
    `🔹 <b>Mentor bo'lish</b> — tajribali dasturchilar uchun\n` +
    `🔹 <b>Stajirovka</b> — yangi boshlovchilar uchun\n\n` +
    `📱 "Web App ochish" tugmasini bosib, barcha imkoniyatlardan foydalaning!\n\n` +
    `📞 Murojaat: @itjobs_support`,
    { parse_mode: 'HTML' },
  );
}
...
export async function handleStats(ctx: Context) {
  try {
    ...
    await ctx.reply('❌ Statistikani olishda xatolik yuz berdi. Keyinroq urinib ko\'ring.');
  }
}
